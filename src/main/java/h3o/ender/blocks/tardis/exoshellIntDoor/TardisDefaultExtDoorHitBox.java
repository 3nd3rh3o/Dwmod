...
public class TardisDefaultExtDoorHitBox extends AbstractGlassBlock {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand,
        ...
        if (!world.isClient) {
            ...
            Tardis trds = DimensionalStorageHelper.getTardis(world.getServer(), pos);
            if (trds == null) {
                world.setBlockState(pos, Blocks.AIR.getDefaultState(), 3);
                return ActionResult.PASS;
            } else {
                return trds.interactAt(player,
                        hit.getPos().subtract(pos.toCenterPos()).rotateY((float)(Math.toRadians(180) - Math.toRadians(trds.getDataTracker().get(Tardis.EXOSHELL_ROT)))), hand);
            }
        }
        return ActionResult.PASS;
    }
